...
/**
 * Do not merge injectable services from our parent class
 * @type {boolean} false
 * @final
 * @static
 */
AbstractDirective.$mergeInject = false;
...
angular.inherits(AbstractDirective, AbstractViewableService, {
	...
	/**
	 * The variable name of the controller bound to the scope object
	 * @type {string|null}
	 */
	controllerAs: null,
	...
	observeExposed: function (expose) {
		AbstractViewableService.prototype.observeExposed.apply(this, arguments);

		// directives can be re-used
	 ...
	 * @param {Object} ctrl The directive controller
	 * @return {void}
	 */
	link: function(scope, elem, attr, ctrl) {
		...
		if (!(this.expose instanceof Array)) {
			...
			expose = this.expose.slice();
		}
		if (ctrl && ctrl.expose instanceof Array) {
			expose = expose.concat(ctrl.expose);
		}
		...
		if (!(this.watch instanceof Array)) {
			...
			watch = this.watch.slice();
		}
		if (ctrl && ctrl.watch instanceof Array) {
			watch = watch.concat(ctrl.watch);
		}

		this.traverseSuper();

		if (ctrl) {
			this.traverseSuper(ctrl);
		}

		this.initScope(scope, expose, watch);
	},
